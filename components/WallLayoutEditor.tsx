...
export function WallLayoutEditor({
  ...
  async function assignSlot(slotKey: string, productId: number | null) {
    setLoading(true);
    // If filling a slot, optionally clear that slot's previous occupant.
    ...
    if (productId === null && occupant) {
      ...
      // If product was previously in another slot, just patch (DB has one slot per product)
      // If another product was here, kick it out first
      ...
      >
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            {current && (
              <div className="text-xs opacity-60 mt-1">Currently: {current.title}</div>
            )}
          </div>
              ...
              >
                <div className="relative w-full aspect-[4/5] bg-muted border border-ink">
                  ...
                  {p.wallSlot && p.wallSlot !== slotKey && (
                    <div className="absolute top-1 right-1 bg-ink text-paper text-[8px] uppercase tracking-widest px-1 py-0.5">
                      In use
                    </div>
                  )}
                </div>
                <div className="mt-1 text-[10px] uppercase tracking-widest truncate font-bold">
                  {p.title}
                </div>
              </button>
            );
